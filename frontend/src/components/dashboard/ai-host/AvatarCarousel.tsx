    ...
    <div className="flex min-h-0 flex-1 gap-2.5">
      ...
      <div className="relative min-h-[160px] w-[42%] min-w-0 overflow-hidden rounded-xl border border-blue-400/40 bg-[#080e1a]">
        ...
        <div className="absolute inset-x-0 bottom-0 space-y-0.5 p-2.5">
          <p className="truncate text-[13px] font-bold text-white">{selectedAvatar.name}</p>
          <p className="truncate text-[9px] text-slate-300">
            {selectedAvatar.gender === "male" ? "Male" : "Female"} · {selectedAvatar.role}
          </p>
        </div>
      </div>
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-1.5 overflow-y-auto [scrollbar-width:thin]">
          ...
          return (
            <button
              ...
              onClick={() => {
                ...
                showToast(
                  av.gender === "female" ? `Host: ${av.name} · suara perempuan` : `Host: ${av.name} · ${av.gender}`,
                );
              }}
            ...
            >
              <span
                ...
                style={{
                  backgroundImage: `url('${av.imageProfile}')`,
                  backgroundPosition: "center 30%",
                  backgroundSize: "340%",
                }}
              />
              <span className="min-w-0 flex-1">
                <span className="block truncate text-[12px] font-bold text-white">{av.name}</span>
              ...
              <span
                className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${
                  isSelected ? "border-blue-300 bg-blue-500 text-white" : "border-white/10 text-transparent"
                }`}
              >
        ...
        <div className="flex w-full items-center gap-2.5 rounded-xl border border-dashed border-[#3a455c] bg-[#0a101c]/70 px-2 py-1.5">
          ...
          <span className="min-w-0 flex-1">
            <span className="block truncate text-[12px] font-bold text-slate-300">Coming Soon</span>
            <span className="block truncate text-[9px] text-slate-600">Host AI baru segera hadir</span>
          </span>
        </div>
      </div>
    </div>
