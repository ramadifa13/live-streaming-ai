...
export async function oauthRoutes(server: FastifyInstance) {
  ...
  server.get<{ Querystring: { platform: string } }>("/api/oauth/authorize", async (request, reply) => {
    ...
    const params = new URLSearchParams({
      ...
      state,
      ...(!isShopee ? { code_challenge: codeChallenge, code_challenge_method: "S256" } : {}),
    });
  ...
  server.get<{ Querystring: { code?: string; state?: string; error?: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!tokenRes.ok) {
          server.log.error(`[OAuth] Token exchange failed for ${platform}: ${await tokenRes.text()}`);
          return reply.redirect(`${frontendUrl}/dashboard?oauth_error=token_exchange_failed`);
        }
          ...
          try {
            ...
            if (profileRes.ok) {
              ...
              if (platform.toLowerCase().includes("tiktok")) {
                ...
                username = String(u.username || u.open_id || "");
                ...
                username = String(profile.username || profile.login || profile.name || "");
                displayName = String(profile.name || profile.username || "");
                avatarUrl = String(profile.avatar_url || profile.profile_picture_url || "");
              }
            }
          } catch {}
        }
        ...
        const session: OAuthSession = {
          platform,
          isConnected: true,
          username: username || `${platform.replace(/\s/g, "").toLowerCase()}_user`,
          displayName: displayName || `${platform} Account`,
          storeName: displayName || `${platform} Official Store`,
          avatarUrl,
          followers,
          rating: 0,
          status: "active",
          ingestUrl: config.rtmpIngestUrl || "",
          streamKey,
          accessToken,
          refreshToken,
          expiresAt,
        ...
        return reply.redirect(
          `${frontendUrl}/dashboard?oauth_success=${encodeURIComponent(platform)}&display=${encodeURIComponent(session.displayName)}`,
        );
      } catch (err) {
  ...
  server.post("/api/oauth/connect", async (request, reply) => {
    ...
    if (config && !config.clientId)
      server.log.warn(`[OAuth] ${platform}: OAuth env keys not configured — using manual connect.`);
    const streamKey = "";
    const session: OAuthSession = {
      platform,
      isConnected: true,
      username: username || `${platform.toLowerCase().replace(/[^a-z0-9]/g, "")}_user`,
      displayName: displayName || `${platform} Official Store`,
      storeName: storeName || "Official Brand Store Indonesia",
      avatarUrl: avatarUrl || "",
      followers: 0,
      rating: 0,
      status: "active",
  ...
  server.get("/api/oauth/config-status", async () => {
    ...
    return {
      ...
      data: platforms.map((p) => ({
        platform: p.name,
        configured: p.envVars.every((k) => !!process.env[k]),
        missingEnvVars: p.envVars.filter((k) => !process.env[k]),
        connected: !!sessionStore[p.name]?.isConnected,
      })),
    };
  });
}
