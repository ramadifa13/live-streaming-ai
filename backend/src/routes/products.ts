...
return {
   ...
   // POST bulk import via CSV file upload
   server.post(
     "/api/products/bulk-csv",
     async (request, reply) => {
       // Get uploaded file
       ...
       // Validate against the same schema used for JSON bulk
       const parsed = bulkProductSchema.safeParse({ products });
       if (!parsed.success) {
         reply.code(400);
         return { success: false, error: parsed.error.flatten() };
       }
       ...
       try {
         ...
         return {
           success: false,
           error: err.message || "Failed to bulk import products via CSV",
         };
       }
     }
   );
