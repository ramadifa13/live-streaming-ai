...
def musetalk_visual_params():
    ...
    extra_margin = int(os.environ.get("MUSETALK_EXTRA_MARGIN", "6"))
    upper_boundary_ratio = float(os.environ.get("MUSETALK_UPPER_BOUNDARY", "0.53"))
    return {
        ...
        "left_cheek_width": int(os.environ.get("MUSETALK_CHEEK_WIDTH", "65")),
        "right_cheek_width": int(os.environ.get("MUSETALK_CHEEK_WIDTH", "65")),
    }
...
def square_pad_face(crop: np.ndarray):
    ...
    if top == 0 and bottom == 0 and left == 0 and right == 0:
        return crop, (0, 0, 0, 0)
    padded = cv2.copyMakeBorder(crop, top, bottom, left, right, cv2.BORDER_REPLICATE)
...
def resize_generated_to_bbox(generated, bbox, square_pad: bool = True) -> np.ndarray:
    ...
    try:
        ...
        for i, frame in enumerate(frames):
            try:
                ...
                try:
                    bbox = face_align_model.get_detections_for_batch(
                        np.asarray([frame])
                    )
        ...
        if failed_frames > 0:
            print(
                f"[Landmarks] WARNING: {failed_frames}/{len(frames)} frames failed landmark extraction"
            )
        return coords_list
    except Exception as e:
        import traceback
        ...
        traceback.print_exc()
        print(
            "[Landmarks] CRITICAL: Falling back to center bbox - lip-sync will NOT work correctly!"
        )
...
def _cycle_state_path(args) -> str:
    ...
    if explicit:
        return explicit
    return os.path.join(
        getattr(args, "result_dir", "./results") or "./results", "cycle_state.json"
    )
    ...
    if cache_key not in _models_cache:
        with _lock:
            if cache_key not in _models_cache:
                ...
                print(
                    f"[MuseTalk-Cache] 🚀 Loading all models to {device} (fp16={use_float16})..."
                )
                ...
                print(
                    f"  - VAE: {vae.__class__.__name__}, device={_nn_device(getattr(vae, 'vae', vae))}"
                )
                print(
                    f"  - UNet: {unet.model.__class__.__name__}, device={_nn_device(unet.model)}"
                )
                print(f"  - PE: {pe.__class__.__name__}, device={_nn_device(pe)}")
                print(
                    f"  - Whisper: {whisper.__class__.__name__}, device={_nn_device(whisper)}"
                )
                ...
                print(
                    "[MuseTalk-Cache] ✅ All models loaded and validated successfully"
                )
...
def _get_avatar_materials(
    ...
    with _lock:
        ...
        print(
            f"[AvatarCache] ✅ Avatar {input_basename} cached ({len(frame_list)} frames, {len(input_latent_list_cycle)} cycles)."
        )
    ...
    for task_id in inference_config:
        try:
            ...
            bbox_shift = (
                vis["bbox_shift"]
                if args.version == "v15"
                else task_info.get("bbox_shift", args.bbox_shift)
            )
            ...
            delay_frame = _load_cycle_offset(video_path, args) % max(
                1, len(input_latent_list_cycle)
            )
            gen = datagen(
            ...
            if ffseg_writer is not None:
                ...
                try:
                    ...
                    print(
                        f"[MuseTalk-Fast] 📦 Raw ffseg siap: {written} ({ffseg_writer.frame_count} frames)"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ffmpeg_path", type=str, default="", help="Path to ffmpeg executable"
    )
    parser.add_argument("--gpu_id", type=int, default=0, help="GPU ID to use")
    parser.add_argument(
        "--vae_type", type=str, default="sd-vae-ft-mse", help="Type of VAE model"
    )
    parser.add_argument(
        "--unet_config",
        type=str,
        default="./models/musetalkV15/musetalk.json",
        help="Path to UNet config",
    )
    parser.add_argument(
        "--unet_model_path",
        type=str,
    ...
    parser.add_argument(
        "--use_saved_coord", action="store_true", help="Use saved coordinates"
    )
    parser.add_argument("--saved_coord", action="store_true", help="Save coordinates")
    parser.add_argument(
        "--use_float16", action="store_true", default=True, help="Use float16"
    )
    parser.add_argument(
